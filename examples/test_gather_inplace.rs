//! Integration test for gather_inplace.
//!
//! Each rank r pre-writes r*10 into its in-place slot at offset r of the shared
//! buffer. Only rank 0 (root) calls gather_inplace; non-root ranks call the
//! regular gather to contribute their value.
//!
//! After the call, rank 0 asserts the buffer equals [0, 10, 20, 30].
//!
//! Run with: mpiexec -n 4 cargo run --example test_gather_inplace
...
fn main() -> Result<()> {
    ...
    if rank == 0 {
        ...
        assert_eq!(
            data, expected,
            "gather_inplace: rank 0 expected {:?} but got {:?}",
            expected, data
        );
        println!("gather_inplace: {:?}", data);
    } else {
        // Non-root: use regular gather with a single-element send buffer.
