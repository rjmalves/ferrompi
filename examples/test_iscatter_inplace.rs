//! Integration test for iscatter_inplace.
//!
//! Root (rank 0) pre-populates a 4-element buffer [0, 10, 20, 30]. After
//! iscatter_inplace(root=0), rank 0 retains its own slot (data[0] == 0) and each
//! non-root rank receives a single-element buffer matching its slot value.
//!
//! Run with: mpiexec -n 4 cargo run --example test_iscatter_inplace
...
fn main() -> Result<()> {
    ...
    if rank == 0 {
        ...
        assert_eq!(
            data[0], 0,
            "iscatter_inplace: rank 0 expected data[0]==0 but got {}",
            data[0]
        );
        println!("iscatter_inplace: rank 0 retained slot data[0]={}", data[0]);
    } else {
        ...
        assert_eq!(
            data[0], expected,
            "iscatter_inplace: rank {} expected data[0]=={} but got {}",
            rank, expected, data[0]
        );
        println!("iscatter_inplace: rank {} received {}", rank, data[0]);
    }
